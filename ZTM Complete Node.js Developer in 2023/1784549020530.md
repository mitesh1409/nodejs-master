...
first takes 10 milli seconds,  
second takes 20 milli seconds &  
third takes 30 milli seconds  
to complete.
...
When all the promises inside `Promise.all` are resolved,  
it executes the callback in `then`.  
If anyone of them fails then it executes the callback in `catch`.  

When all the promises inside `Promise.allSettled` are settled,  
it executes the callback in `then`.  
...
When a promise is settled, it can be in either Fulfilled/Resolved or Rejected status.
