# Complete Node.js Developer by ZTM Udemy > Appendix: How JavaScript Works
...
An assembly language (or ASM) is a low-level programming language designed to communicate directly with a computer’s hardware. It acts as a human-readable abstraction over binary machine code (1s and 0s), replacing numeric opcodes with text-based mnemonics.

Assembly language is a low-level language tied directly to a specific physical hardware architecture, while WebAssembly (Wasm) is a platform-independent bytecode format designed to run at near-native speeds inside a sandboxed virtual machine.

Despite sharing the word "Assembly," they serve entirely different ecosystems. Engineers use assembly language for bare-metal hardware control, whereas they use WebAssembly to run high-performance code on the web and modern cloud environments.
...
**Call Stack**  
A place to keep track of where we are in the code execution so that we can run the code in order.  
